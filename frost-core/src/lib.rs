...
pub struct SigningPackage<C: Ciphersuite> {
    ...
    message: Vec<u8>,
    /// Optional adaptor statement (public point Y) for adaptor signatures.
    ///
    /// When present, the challenge will be computed as H(R + Y, PK, m) instead
    /// of H(R, PK, m), enabling adaptor signature protocols where the final
    /// signature can only be completed by someone who knows the discrete log
    /// of Y (the adaptor witness y).
    #[getter(skip)]
    #[cfg_attr(feature = "serde", serde(default))]
    adaptor_statement: Option<serialization::SerializableElement<C>>,
}
    ...
    C: Ciphersuite,
{
    ...
    pub fn new(
        ...
        SigningPackage {
            ...
            adaptor_statement: None,
        }
    }

    /// Create a new `SigningPackage` with an adaptor statement for adaptor signatures.
    ///
    /// The `adaptor_statement` is the public point Y for which the signer will create
    /// an adaptor pre-signature. The challenge is computed as H(R + Y, PK, m) instead
    /// of H(R, PK, m). The pre-signature can then be adapted into a valid signature
    /// by adding the discrete log of Y (the adaptor witness).
    ///
    /// The `signing_commitments` are sorted by participant `identifier`.
    pub fn new_with_adaptor(
        signing_commitments: BTreeMap<Identifier<C>, round1::SigningCommitments<C>>,
        message: &[u8],
        adaptor_statement: <C::Group as Group>::Element,
    ) -> SigningPackage<C> {
        SigningPackage {
            header: Header::default(),
            signing_commitments,
            message: message.to_vec(),
            adaptor_statement: Some(serialization::SerializableElement(adaptor_statement)),
        }
    }

    /// Returns the adaptor statement (public point Y) if this is an adaptor signing package.
    pub fn adaptor_statement(&self) -> Option<<C::Group as Group>::Element> {
        self.adaptor_statement.as_ref().map(|e| e.0)
    }

    /// Returns whether this is an adaptor signing package.
    pub fn is_adaptor(&self) -> bool {
        self.adaptor_statement.is_some()
    }

    /// Computes the commitment point to use for challenge computation.
    ///
    /// For standard signatures, this returns R (the group commitment).
    /// For adaptor signatures, this returns R' = R + Y where Y is the adaptor statement.
    pub fn challenge_commitment(&self, group_commitment: &<C::Group as Group>::Element) -> <C::Group as Group>::Element {
        match &self.adaptor_statement {
            Some(adaptor_y) => *group_commitment + adaptor_y.0,
            None => *group_commitment,
        }
    }
...
impl<C> Debug for SigningPackage<C>
where
    C: Ciphersuite,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut debug_struct = f.debug_struct("SigningPackage");
        debug_struct.field("signing_commitments", &self.signing_commitments);
        debug_struct.field("message", &hex::encode(&self.message));
        // For adaptor_statement, we serialize it if present to avoid Debug bound issues
        if let Some(ref adaptor) = self.adaptor_statement {
            if let Ok(serialized) = <C::Group as Group>::serialize(&adaptor.0) {
                debug_struct.field("adaptor_statement", &Some(hex::encode(serialized.as_ref())));
            } else {
                debug_struct.field("adaptor_statement", &Some("<serialization error>"));
            }
        } else {
            debug_struct.field("adaptor_statement", &Option::<()>::None);
        }
        debug_struct.finish()
    }
}
...
pub fn verify_signature_share<C: Ciphersuite>(
    ...
    // Compute the per-message challenge.
    // For adaptor signatures, use R' = R + Y as the commitment point.
    let challenge_point = signing_package.challenge_commitment(&group_commitment.clone().to_element());
    let challenge = <C>::challenge(
        &challenge_point,
        verifying_key,
